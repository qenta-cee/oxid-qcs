...
(function ($) {
  var ddQentaPayment = {
    ...
    _create: function () {
      ...
      if ($('#qentaCheckoutSeamless_ccardIframeContainer').length > 0) {
        this.paymentTypes['qcs_ccard'].iframe = qenta.buildIframeCreditCard(
          'qentaCheckoutSeamless_ccardIframeContainer',
          '100%',
          '160px'
        );
      }
      if ($('#qentaCheckoutSeamless_ccard-motoIframeContainer').length > 0) {
        this.paymentTypes['qcs_ccard-moto'].iframe =
          qenta.buildIframeCreditCard(
            'qentaCheckoutSeamless_ccard-motoIframeContainer',
            '100%',
            '160px'
          );
      }
      ...
      // jquery 1.8+
      if (!elEvents) elEvents = jQuery._data(el[0], 'events');
      var oldSubmit = false;
      if (elEvents.submit) {
    ...
    submitPayment: function (oForm) {
      ...
      if (
        blStoredvalue != '0' &&
        typeof this.paymentTypes[sPaymentType] != 'undefined' &&
        this.paymentTypes[sPaymentType].iframe === null
      ) {
        return true;
      }
      ...
      if (oGatewayState === 0) {
        ...
        if (oGatewayState.getErrors) {
          for (var x in oGatewayState) {
            ...
            else this.showError(oGatewayState[x].message);
          }
        }
    ...
    usePaymentGateway: function (paymentType, oForm) {
      ...
      for (var fieldName in paymentTypeObject.fields) {
        ...
        if (formField.length == 1) {
          paymentInformation[paymentTypeObject.fields[fieldName]] =
            formField[0].value;
        }
      }

      if (
        $('#' + paymentType + '_stored').val() == 0 ||
        paymentTypeObject.iframe !== null
      ) {
        return this.qpaySeamlessRequest(
          paymentInformation,
          function (oResponse) {
            if (oResponse.getStatus() == 0) {
              ...
              var anonymizedPaymentInformation =
                oResponse.getAnonymizedPaymentInformation();
              ...
              if (paymentTypeObject.iframe === null) {
                ...
                for (var x in errors) {
                  if (errors[x].consumerMessage)
                    self.showError(errors[x].consumerMessage);
    ...
    qpaySeamlessRequest: function (paymentInformation, callback) {
      try {
        ...
        return DataStorage.storePaymentInformation(
          paymentInformation,
    ...
    showError: function (message, type) {
      ...
      var actPaymentErrorList = $(
        '#qentaErrorList',
        actPaymentRadio.parents('dl')
      );
